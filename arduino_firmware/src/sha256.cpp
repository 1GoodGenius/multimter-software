...
// Minimal SHA-256 implementation (small, not optimized). Public-domain style for embedded use.
// Note: Suitable for moderate-size inputs (timestamp or short buffers). For large inputs, test performance.
...
static void sha256_transform(SHA256_CTX* ctx, const uint8_t data[]) {
  ...
  for (uint8_t i = 16; i < 64; ++i) w[i] = small_sigma1(w[i-2]) + w[i-7] + small_sigma0(w[i-15]) + w[i-16];
